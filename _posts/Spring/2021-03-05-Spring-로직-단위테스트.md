...
// 이클립스에서 Ctrl + Space 를 누르면 Code Assist가 동작하여 처음 몇자만 입력하면 필요한 클래스의 import 나 사용 가능한 메소드를 추천 또는 자동완성을 해 줍니다. 
// 하지만 static import 는 기본적으로 자동으로 인식하지 못합니다.
...
`given()`은 static메소드입니다. import문을 보면 **`org.mockito.BDDMockito` 클래스의 static 메소드**인것을 알 수 있습니다. 
